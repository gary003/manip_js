...
const int_vals = vals.filter((val) => ![undefined, null].includes(val)).map((v) => parseInt(v))
...
let countries = users.map((val) => val.country).filter((val, i, me) => me.indexOf(val) === i)
...
const group = users.reduce((group, us) => {
